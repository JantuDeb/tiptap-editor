...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({
  ...
  <>
    ...
    <div className="tiptap-image-upload-content">
      ...
      <span className="tiptap-image-upload-subtext">
        Maximum {limit} file{limit === 1 ? "" : "s"}, {maxSize / 1024 / 1024}
        MB each.
      </span>
    </div>
  </>
...
export const VideoUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  const handleUpload = async (files: File[]) => {
    ...
    if (urls.length > 0) {
      ...
      if (isValidPosition(pos)) {
        const videoNodes = urls.map((videoUrl) => ({
  ...
  return (
    <NodeViewWrapper className="tiptap-image-upload" onClick={handleClick}>
      {!hasFiles && (
      ...
      {!hasFiles && (
        <div className="tiptap-video-upload-url-form">
          <Input
            type="text"
