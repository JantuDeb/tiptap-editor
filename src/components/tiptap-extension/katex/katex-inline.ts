...
export const InlineMath = Node.create<InlineMathOptions>({
    ...
    return [
      new InputRule({
        ...
        handler: ({ state, range, match }) => {
          ...
          // adjust range to only replace the $...$
  ...
  addNodeView() {
    ...
    return ({ node, getPos, editor }) => {
      ...
      function renderMath() {
        try {
          katex.render(node.attrs.latex, renderEl, katexOptions);
          renderEl.classList.remove("math-error");
        } catch {
          renderEl.textContent = node.attrs.latex;
          renderEl.classList.add("math-error");
        }
      }
